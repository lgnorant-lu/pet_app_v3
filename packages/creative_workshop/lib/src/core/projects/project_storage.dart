...
class LocalProjectStorage implements ProjectStorage {
  ...
  /// 获取项目文件路径
  String _getProjectFilePath(String projectId) => path.join(_storageDirectory, '$projectId.json');
  ...
  Future<bool> saveProject(CreativeProject project) async {
    try {
      ...
      final jsonString = const JsonEncoder.withIndent('  ').convert(projectJson);
      
      if (kIsWeb) {
        // Web平台使用localStorage或IndexedDB
        // 这里简化处理，实际应该使用web存储API
        debugPrint('Web存储: 保存项目 ${project.id}');
        return true;
      } else {
  ...
  Future<CreativeProject?> loadProject(String projectId) async {
    try {
      ...
      if (kIsWeb) {
        // Web平台从localStorage或IndexedDB加载
...
class MemoryProjectStorage implements ProjectStorage {
  ...
  @override
  Future<bool> projectExists(String projectId) async => _projects.containsKey(projectId);
