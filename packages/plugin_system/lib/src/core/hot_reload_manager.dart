...
class HotReloadManager {
  ...
  Future<void> stopWatching() async {
    ...
    for (final StreamSubscription<FileSystemEvent> watcher in _watchers.values) {
  ...
  Future<void> _watchPluginPath(String path) async {
    ...
    if (!await directory.exists()) {
      throw PluginNotFoundException('Plugin path not found: $path');
    }
    
    // 简化实现：监听目录变更
    final Stream<FileSystemEvent> events = directory.watch(recursive: true);
    final StreamSubscription<FileSystemEvent> subscription = events.listen(
      (FileSystemEvent event) => _handleFileSystemEvent(event),
  ...
  void _handleFileSystemEvent(FileSystemEvent event) {
    // TODO(High): [Phase 2.9.1] 实现文件系统事件处理
    // 需要实现：
    // 1. 事件类型分析
    // 2. 插件文件识别
    // 3. 自动重载触发
  ...
  Future<void> _savePluginState(String pluginId) async {
    ...
    _stateSnapshots[pluginId] = PluginStateSnapshot(
      pluginId: pluginId,
      state: state,
      config: <String, dynamic>{}, // TODO: 获取实际配置
