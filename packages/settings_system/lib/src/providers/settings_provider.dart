...
class SettingsNotifier extends StateNotifier<Settings> {
  ...
  Future<void> updateMemoryLimit(int memoryLimitMB) async {
    ...
    state = state.copyWith(app: state.app.copyWith(memoryLimitMB: memoryLimitMB));
  }
  ...
  Future<void> updateCacheStrategy(CacheStrategy cacheStrategy) async {
    ...
    state = state.copyWith(app: state.app.copyWith(cacheStrategy: cacheStrategy));
  }

  /// 更新自动更新插件
  Future<void> updateAutoUpdate(bool autoUpdate) async {
    await _settingsService.updateAutoUpdate(autoUpdate);
    state = state.copyWith(plugins: state.plugins.copyWith(autoUpdate: autoUpdate));
  }

  /// 更新允许Beta插件
  Future<void> updateAllowBetaPlugins(bool allowBetaPlugins) async {
    await _settingsService.updateAllowBetaPlugins(allowBetaPlugins);
    state = state.copyWith(plugins: state.plugins.copyWith(allowBetaPlugins: allowBetaPlugins));
  }

  /// 更新插件商店URL
  Future<void> updateStoreUrl(String storeUrl) async {
    await _settingsService.updateStoreUrl(storeUrl);
    state = state.copyWith(plugins: state.plugins.copyWith(storeUrl: storeUrl));
  }
